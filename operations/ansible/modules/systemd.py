...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            root_privileges=dict(type='bool'),
        ),
    ...
    if module.params['daemon_reload'] and not module.check_mode:
        (rc, out, err) = module.run_command("%s%s daemon-reload" % (root_command, systemctl))
        if rc != 0:
            module.fail_json(msg='failure %d during daemon-reload: %s' % (rc, err))

    # Run daemon-reexec
    if module.params['daemon_reexec'] and not module.check_mode:
        (rc, out, err) = module.run_command("%s%s daemon-reexec" % (root_command, systemctl))
        if rc != 0:
    ...
    if unit:
        ...
        (rc, out, err) = module.run_command("%s%s show '%s'" % (root_command, systemctl, unit))
        ...
        # Workaround for https://github.com/ansible/ansible/issues/71528
            ...
            (rc, out, err) = module.run_command("{root_command}{systemctl} list-unit-files '{unit_search}*'".format(systemctl=systemctl, unit_search=unit_search))
            is_systemd = unit_search in out

            (rc, out, err) = module.run_command("{root_command}{systemctl} is-active '{unit}'".format(root_command=root_command, systemctl=systemctl, unit=unit))
            ...
            (rc, out, err) = module.run_command("%s%s is-enabled '%s'" % (root_command, systemctl, unit))
            if out.strip() in valid_enabled_states:
                is_systemd = True
            else:
                # fallback list-unit-files as show does not work on some systems (chroot)
                # not used as primary as it skips some services (like those using init.d) and requires .service/etc notation
                (rc, out, err) = module.run_command("%s%s list-unit-files '%s'" % (root_command, systemctl, unit))
                if rc == 0:
                    is_systemd = True
                else:
        ...
        if module.params['masked'] is not None:
            ...
            if masked != module.params['masked']:
                ...
                if not module.check_mode:
                    (rc, out, err) = module.run_command("%s%s %s '%s'" % (root_command, systemctl, action, unit))
                    if rc != 0:
        ...
        if module.params['enabled'] is not None:
            ...
            enabled = False
            (rc, out, err) = module.run_command("%s%s is-enabled '%s' -l" % (root_command, systemctl, unit))
